...
object DateUtil {
    ...
    object WithServer {
        fun asStringOnlyDate(now: LocalDate): String {
            val monthValue = if (now.monthValue < 10) "0${now.monthValue}" else now.monthValue.toString()
    ...
    object WithUser {
        ...
        fun hourMinute(createdAt: LocalDateTime): String {
            ...
            val hour = if (toLocalTime.hour / 10 >= 1) toLocalTime.hour.toString() else "0${toLocalTime.hour}"
            val minutes = if (toLocalTime.minute < 10) "0${toLocalTime.minute}" else toLocalTime.minute.toString()
